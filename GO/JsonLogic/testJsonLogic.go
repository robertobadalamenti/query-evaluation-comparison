...
type User struct {
	...
	IsMusician     bool
	Instrument     string
	GroupedField1  string
	GroupedField4  string
	GroupedField2  string
	Height         int
	Gender         string
	Alarm          string
	Birthdate      string
...
func Test() {
	...
	// Regola JSONLogic che controlla se Age >= 18
	logicJSON := `
{
  "and": [
    {
  ...
  "dateBetween": [
	...
	for range 1000 {
		for _, in := range users {
			...
			dataBytes, _ := json.Marshal(in)
			var dataMap map[string]any
			json.Unmarshal(dataBytes, &dataMap)

			start := time.Now()
			result, _ := jsonlogic.ApplyInterface(logic, dataMap)

			elapsed := time.Since(start)
			tot += elapsed
			if in.expectedResult != result {
